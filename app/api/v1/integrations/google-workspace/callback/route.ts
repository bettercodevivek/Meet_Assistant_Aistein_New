...
export async function GET(request: NextRequest) {
  try {
    ...
    const refreshToken = tokens.refresh_token || existingUser?.googleWorkspaceIntegration?.refreshToken;
    ...
    return NextResponse.redirect(
      `${frontendUrl}/dashboard/integrations?success=google_workspace`
    );
  } catch (error) {
